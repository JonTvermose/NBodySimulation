...
public class BodySystem {
	...
	public void resetBodies(){		
		...
		double px = 1.47e17; // Earth orbit distance from sun
		double py = 0;
	...
	public void addRandomBodies(int n){
		for (int i = 0; i < n; i++) {
			double px = Math.abs(1e18*exp(-1.8)*(.5-Math.random()));
			double mass = Math.abs(Math.random()*9.393e20*exp(1.8)); // Up to the mass of Ceres
			Color color = Color.ANTIQUEWHITE;
			bodies.add(new Asteroid(px, mass, color));
		}
	}

	public synchronized void addBody(Body b){
		if(b instanceof Star || b instanceof Planet){
			gravityBodies.add(b);
		} else {
			bodies.add(b);			
		}
